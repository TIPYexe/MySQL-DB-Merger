...
def mergeTabels(db_to_merge, db_main, table1, table2, key_table_1, key_table_2):
    ...
    sameIdConflictIssueSolver(mydb1, mydb2, table_1_merge, table_2_merge, table_1_main, key_table_1, key_table_2)
    # 0 = id, 2 = type, 0 = skips no columns | 3 = providerId, 4 = regionId, 1 = skips the 1st column
    mergeTheNewData(mydb1, mydb2, table_1_merge, table_1_main, 0, 2, 0)
    mergeTheNewData(mydb1, mydb2, table_2_merge, table_2_main, 3, 4, 1)
    mydb2.commit()


# will assign a new and unique ID to each
def sameIdConflictIssueSolver(mydb1, mydb2, table_1_merge, table_2_merge, table_1_main, key_1, key_2):
    mycursos_1_merge = mydb1.cursor()
    mycursor_1_main = mydb2.cursor()
    mycursor_2_main = mydb2.cursor()
    ...
    if len(myresult_1_main) > len(myresult_1_merge):
        newId_1 = myresult_1_main[len(myresult_1_main) - 1][0] + 1
    else:
        newId_1 = myresult_1_merge[len(myresult_1_merge) - 1][0] + 1
...
def mergeTheNewData(mydb1, mydb2, table_1_merge, table_1_main, i, j, skip):
    mycursor1 = mydb1.cursor()
    ...
    columnsRaw = mycursor1.fetchall()
    columnNames = list(map(lambda x: re.sub("[(),']", "", str(x)), columnsRaw))
    columnCommand = ", ".join(columnNames[skip:])
...
# 1 = branch to merge
# 2 = main
mergeTabels("urbanair-side", "urbanair-main", "providers", "providerregions", "id", "providerId")
